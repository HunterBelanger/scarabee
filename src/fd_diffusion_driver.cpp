...
void set_x_current_diff(DiffusionGeometry& geom, Eigen::SparseMatrix<double>& M, const std::size_t g, const std::size_t m) {
  ...
  if (op_mm1 && op_mp1) {
    ...
    const double a = -(2. / dx_m) * (d_m*d_mp1/(d_m + d_mp1)); // for flux m+1
    const double c = -(2. / dx_m) * (d_m*d_mm1/(d_m + d_mm1)); // for flux m-1
    ...
    const double b = -c + (2.*d_m*R/(dx_m*(4.*d_m+R))); // for flux m
    ...
    const double R = (1. - alb) / (1. + alb);
    const double b = -a + (2.*d_m*R/(dx_m*(4.*d_m+R))); // for flux m
...
void set_y_current_diff(DiffusionGeometry& geom, Eigen::SparseMatrix<double>& M, const std::size_t g, const std::size_t m) {
  ...
  std::tie(tile_mm1, op_mm1) = geom.neighbor(m, DiffusionGeometry::Neighbor::YN);
  std::tie(tile_mp1, op_mp1) = geom.neighbor(m, DiffusionGeometry::Neighbor::YP);
  ...
  if (op_mm1 && op_mp1) {
    ...
    // Now we compute the coefficient for each flux term
    const double a = -(2. / dy_m) * (d_m*d_mp1/(d_m + d_mp1)); // for flux m+1
    const double c = -(2. / dy_m) * (d_m*d_mm1/(d_m + d_mm1)); // for flux m-1
    const double b = -(a + c); // for flux m
    ...
    const double alb = tile_mp1.albedo.value();
    const double R = (1. - alb) / (1. + alb);
    const double b = -c + (2.*d_m*R/(dy_m*(4.*d_m+R))); // for flux m
    ...
    const double alb = tile_mm1.albedo.value();
    const double R = (1. - alb) / (1. + alb);
    const double b = -a + (2.*d_m*R/(dy_m*(4.*d_m+R))); // for flux m
...
void set_z_current_diff(DiffusionGeometry& geom, Eigen::SparseMatrix<double>& M, const std::size_t g, const std::size_t m) {
  ...
  std::tie(tile_mm1, op_mm1) = geom.neighbor(m, DiffusionGeometry::Neighbor::ZN);
  std::tie(tile_mp1, op_mp1) = geom.neighbor(m, DiffusionGeometry::Neighbor::ZP);
  ...
  if (op_mm1 && op_mp1) {
    ...
    // Now we compute the coefficient for each flux term
    const double a = -(2. / dz_m) * (d_m*d_mp1/(d_m + d_mp1)); // for flux m+1
    const double c = -(2. / dz_m) * (d_m*d_mm1/(d_m + d_mm1)); // for flux m-1
    const double b = -(a + c); // for flux m
    ...
    const double alb = tile_mp1.albedo.value();
    const double R = (1. - alb) / (1. + alb);
    const double b = -c + (2.*d_m*R/(dz_m*(4.*d_m+R))); // for flux m
    //const double b = -c + (2.*d_m/dy_m); // for zero flux at boundary
    ...
    const double alb = tile_mm1.albedo.value();
    const double R = (1. - alb) / (1. + alb);
    const double b = -a + (2.*d_m*R/(dz_m*(4.*d_m+R))); // for flux m
...
void load_matrix(DiffusionGeometry& geom, Eigen::SparseMatrix<double>& M) {
  ...
  for (std::size_t g = 0; g < geom.ngroups(); g++) {
    for (std::size_t m = 0; m < geom.nmats(); m++) {
      ...
      if (geom.ndims() > 1)
        set_y_current_diff(geom, M, g, m);

      if (geom.ndims() > 2)
        set_z_current_diff(geom, M, g, m);
...
std::tuple<xt::xarray<double>, xt::xarray<double>, std::optional<xt::xarray<double>>, std::optional<xt::xarray<double>>> FDDiffusionDriver::flux() const {
  ...
  if (geom_->ndims() == 1) {
    ...
    flux = xt::zeros<double>({geom_->ngroups(), geom_->nx(), geom_->ny(), geom_->nz()});
  }
