...
std::pair<std::vector<double>, std::vector<double>> group_xs(
    const std::vector<double>& group_bounds,
    const std::function<double(double)>& flux,
    const std::vector<double>& energy, const std::vector<double>& xs) {
  ...
  pndl::Tabulated1D flux_xs_func(pndl::Interpolation::LinLin, energy,
                                 flux_xs_vals);
  ...
  for (std::size_t i = 0; i < group_bounds.size() - 1; i++) {
    ...
    double grp_xs =
        flux_xs_func.integrate(Elow, Ehi) / flux_func.integrate(Elow, Ehi);
...
class PlotLayer : public ImApp::Layer {
 public:
  PlotLayer() {
    ...
    for (auto& engy : energy) {
      engy *= 1.E6;
    }

    MacroFluxSpectrum spec;
    pndl::Tabulated1D tot_xs(pndl::Interpolation::LinLin, energy,
                             nuc.total_xs().xs());
  ...
  void render() override final {
    ...
    ImPlot::PlotLine("Smooth Spectrum", grouped_xss[0].first.data(),
                     grouped_xss[0].second.data(),
                     static_cast<int>(grouped_xss[0].first.size()));
    ImPlot::PlotLine("NR Spectrum", grouped_xss[1].first.data(),
                     grouped_xss[1].second.data(),
                     static_cast<int>(grouped_xss[1].first.size()));
