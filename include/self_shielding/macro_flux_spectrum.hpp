...
class MacroFluxSpectrum {
  ...
  MacroFluxSpectrum(Component thermal, Component epithermal, Component fission,
    ...
    if (thermal_point_ < 0.) {
      throw ScarabeeException(
          "Thermal - epithermal transition energy must be > 0.");
    }

    if (fission_point_ < 0.) {
      throw ScarabeeException(
          "Epithermal - fission transition energy must be > 0.");
    }

    if (fission_point_ <= thermal_point_) {
      throw ScarabeeException(
          "Epithermal-fission transition energy must be > thermal-epithermal "
          "transition energy.");
    }
 ...
 private:
  ...
  struct EvalVisitor {
    ...
    double operator()(const Maxwellian& comp) const { return comp(E); }

    double operator()(const OneOverE& comp) const { return comp(E); }

    double operator()(const Watt& comp) const { return comp(E); }
  ...
  struct SetConstVisitor {
    ...
    void operator()(Maxwellian& comp) const { comp.set_constant(C); }

    void operator()(OneOverE& comp) const { comp.set_constant(C); }

    void operator()(Watt& comp) const { comp.set_constant(C); }
  ...
  void set_constants() {
    ...
    const double thermal_thermal_point_val =
        std::visit(EvalVisitor{thermal_point_}, thermal_);
    const double epithermal_thermal_point_val =
        std::visit(EvalVisitor{thermal_point_}, epithermal_);
    const double epi_const =
        thermal_thermal_point_val / epithermal_thermal_point_val;
    std::visit(SetConstVisitor{epi_const}, epithermal_);

    const double epithermal_fission_point_val =
        std::visit(EvalVisitor{fission_point_}, epithermal_);
    const double fission_fission_point_val =
        std::visit(EvalVisitor{fission_point_}, fission_);
    const double fis_const =
        epithermal_fission_point_val / fission_fission_point_val;
